...
local file_picker = require('opencode.ui.file_picker')
...
function M.save_restore_point(snapshot_id, from_snapshot_id, deleted_files)
  ...
  local snapshot = {
    ...
    files = M._opencode({ 'patch', snapshot_id }).files,
...
function M.get_restore_points()
  ...
  state.restore_points = util.read_json_dir(state.active_session.cache_path .. 'snapshots/') or {}
  return state.restore_points
end
...
function test()
  vim.ui.select(M.get_restore_points(), {
    ...
    format_item = function(item)
      return '📄[+'
        ...
        .. item.id:sub(1, 8)
        .. ' - '
        .. util.time_ago(item.created_at)
        .. (item.from_snapshot_id and (' (from: ' .. item.from_snapshot_id:sub(1, 8) .. ')') or '')
    end,
  }, function(selected)
    if not selected then
      return
    end
    local snapshot = M.restore(selected.id)
    if snapshot then
      vim.notify('Reverted to snapshot: ' .. snapshot.id, vim.log.levels.INFO)
    else
      vim.notify('Failed to revert to snapshot: ' .. selected.id, vim.log.levels.ERROR)
    end
  end)
end
