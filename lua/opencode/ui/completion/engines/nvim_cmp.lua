...
function M.setup(completion_sources)
  ...
  -- Set up completion confirmation hook
  local on_confirm_orig = cmp.get_config().confirmation.get_commit_characters or function()
    return {}
  end
    ...
    callback = function(event)
      ...
      if entry and entry.source.name == 'opencode_mentions' then
        ...
        if item_data and item_data.original_item then
          ...
          completion._on_complete(item_data.original_item)
        end
      end
    end,
  })
