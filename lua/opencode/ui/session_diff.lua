...
---@field review_range? fun(from: string, to?: string): Promise<nil>
local view
...
local function map_actions(buf, scope)
  ...
  for key, entry in pairs(config.keymap.session_diff[scope]) do
    if entry ~= false then
      vim.keymap.set('n', key, actions[entry[1]], {
        buffer = buf,
        silent = true,
        desc = entry.desc,
        nowait = entry.nowait,
...
local function update()
  ...
  if view.mode == 'sides' then
    local before, after = patch.sides(file.patch)
    if not before then
...
function M.activate()
  ...
  if view.range_mode then
    ...
    local review_range = view.review_range --[[@as fun(from: string, to?: string): Promise<nil>]]
    review_range(from, from ~= to and to or nil)
    return
  end
