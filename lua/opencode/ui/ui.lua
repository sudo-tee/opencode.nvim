...
local function refresh_cursor_positions(windows)
  return {
    input = state.save_cursor_position('input', windows and windows.input_win)
      or state.get_cursor_position('input'),
    output = state.save_cursor_position('output', windows and windows.output_win)
...
local function capture_hidden_snapshot(windows)
  ...
  local cursor_positions = refresh_cursor_positions(windows)
...
local function close_windows_impl(windows, should_preserve)
  ...
  if should_preserve then footer.close(true) else renderer.teardown(); footer.close(false) end
  ...
  (should_preserve and FINALIZE.hide or FINALIZE.teardown)(windows, snapshot)
end
