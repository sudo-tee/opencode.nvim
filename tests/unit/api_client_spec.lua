...
describe('api_client', function()
  ...
  it('maps outbound query paths using runtime to_server transform', function()
    ...
    assert.is_not_nil(
      captured_call.url:find('path=/mnt/c/Users/me/repo/main.lua', 1, true)
    )
    assert.is_not_nil(captured_call.url:find('directory=/mnt/c/Users/me/repo', 1, true))
  ...
  it('maps event subscription directory using runtime to_server transform', function()
    ...
    assert.are.equal(
      'http://localhost:8080/event?directory=/mnt/c/Users/me/repo',
      captured_url
    )
