...
describe('persist_state', function()
  ...
  describe('content, cursor, focus and zoom persistence', function()
    it('keeps get_window_state pure, returns window copy, and is tab-safe', function()
      setup_ui()
      create_code_file()
      toggle_wait('visible')
      ...
      local save_cursor_stub = stub(state, 'save_cursor_position')
      local snapshot = api.get_window_state()
      save_cursor_stub:revert()

      assert.equals('visible', snapshot.status)
      assert.stub(save_cursor_stub).was_not_called()
      local original_input_win = state.windows.input_win
