...
describe('V2 protocol operations', function()
  ...
  it('uses direct, location/data, and page response contracts from the V2 fixtures', function()
    ...
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fworkspace', calls[1].request.query)
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fworkspace', calls[2].request.query)
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fworkspace', calls[3].request.query)
  ...
  it('uses endpoint-native permission and question requests and exact 204 responses', function()
    ...
    assert.equals('location%5Bdirectory%5D=%2Fremote%2Fproject', calls[1].query)
    assert.equals('/api/form', calls[2].path)
    assert.equals('/api/session/ses-1/permission/per-1/reply', calls[3].path)
    assert.same({ decision = 'once' }, vim.json.decode(calls[3].body))
  ...
  it('uses the remaining proven project, catalog, filesystem, VCS, and MCP contracts', function()
    ...
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fproject&query=main&type=file', calls[9].query)
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fproject', calls[11].query)
    assert.equals('location%5Bdirectory%5D=%2Fserver%2Fproject', calls[12].query)
  end)
